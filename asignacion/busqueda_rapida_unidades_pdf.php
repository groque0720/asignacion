...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('PLANILLA DE ASIGNACIÓN RESULTADO BUSQUEDA'),0,0,'C');
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones WHERE (id_modelo = ". $modelo['idmodelo'] .") AND (
...
$pdf->Output('BusquedaPlanilaAsignacion_'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
