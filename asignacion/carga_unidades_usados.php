		...
		<?php
			...
			//   20 - bartek // 17 - Peulich // 75 - crespo // 128 - Gonzalez
			  ...
			  if ( $estado['id_estado_usado'] == 1 or $estado['id_estado_usado'] == 4  or in_array($id_usuario,$user_permitidos) ) {
				...
				$SQL="SELECT *, DATEDIFF(DATE(NOW()),fec_recepcion)as ant FROM asignaciones_usados WHERE entregado = 0 AND id_estado =".$estado['id_estado_usado']." ORDER BY interno DESC";
				...
				<?php
				 ...
				 ?>
					...
					<tr class="<?php echo 'fila_'.$estado['id_estado_usado'].'_'.$fila.' '.$libre. ' '.$nc ?>" style="<?php echo $antiguedad_color.' '.$sin_cancelar;  ?>">
						...
						<td class="centrar-texto celda-usado" data-id="<?php echo $usado['id_unidad']; ?>"><?php echo $moneda.number_format($usado['precio_contado'], 0, ',','.'); ?></td>
