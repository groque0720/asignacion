<div class="titulo-modelo centrar-texto">
	<?php echo 'UNIDADES POR CAER POR FALTA DE CANCELACION' ?>
</div>
<?php
...
while ($grupo=mysqli_fetch_array($grupos)) {
	...
	while ($modelo=mysqli_fetch_array($modelos)) {?>
		...
		$SQL="SELECT * FROM view_asignaciones_por_cancelacion WHERE id_modelo = ". $modelo['idmodelo'];
		...
		<tbody  style="page-break-inside: always;" class="lista-unidades listado_asignacion">
			<tr style="page-break-inside: always;" class="<?php echo 'fila_'.$fila.' '.$libre. ' '.$nc.' '.$por_caer_fc.' '.$atp; ?>">
				...
				<td style="page-break-inside: always;" class="centrar-texto  celda" data-id="<?php echo $unidad['id_unidad']; ?>"><?php echo cambiarFormatoFecha($unidad['fec_limite']); ?></td>
			</tr>
