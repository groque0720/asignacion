...
$this->Cell(21,5,'Nro Orden',0,0,'C');
...
$pdf->SetAutoPageBreak(auto,5);
	...
	while ($unidad=mysqli_fetch_array($unidades)) { 
		...
		if ($fec_anterior!=$unidad['fec_playa']) {
			...
			if ($cant_unidades!=1) {
				$acum=$acum+$parcial;
				$pdf->Cell(155,5,'Total Parcial',0,0,'R');
				$pdf->Cell(23,5,'$ '.number_format($parcial, 2, ',','.'),1,1,'R');
				
				$pdf->Cell(155,5,'Acumulado ',0,0,'R');
				$pdf->Cell(23,5,'$ '.number_format($acum, 2, ',','.'),1,1,'R');
		...
		$pdf->Cell(21,5,utf8_decode($unidad['nro_orden']),1,0,'C');
		...
		$pdf->Cell(23,5,'$ '.number_format($unidad['costo'], 2, ',','.'),1,0,'R');
