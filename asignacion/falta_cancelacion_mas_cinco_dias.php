...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('LLEGADAS NO CANCELADAS POR SUCURSAL Y ASESOR'),0,0,'C');
...
$cont_suc=1;
	...
	while ($sucursal=mysqli_fetch_array($sucursales)) {
		...
		while ($usuario=mysqli_fetch_array($usuarios)) {
			...
			$SQL="SELECT * FROM view_asignaciones_llegadas_mas_cinco_dias WHERE id_asesor = ". $usuario['idusuario'];
...
$pdf->Output('FaltaCancelarSucAs'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
