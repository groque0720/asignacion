...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('LLEGADAS NO RETIRADAS Y POR LLEGAR DEL MES ACTUAL'),0,0,'C');
...
$cont_suc=1;
	...
	while ($sucursal=mysqli_fetch_array($sucursales)) {
		...
		$pdf->SetFont('');
		$pdf->Cell(0,5,utf8_decode($sucursal['sucursal']),1,1,'C');


		$SQL="SELECT * FROM usuarios WHERE activo = 1 AND idperfil = 3 AND idsucursal = ".$sucursal['idsucursal'];
		$usuarios=mysqli_query($con, $SQL);

		while ($usuario=mysqli_fetch_array($usuarios)) {


			$SQL="SELECT * FROM asignaciones WHERE id_mes<= MONTH(CURDATE())  AND año <= YEAR(CURDATE()) AND fec_Arribo is null AND borrar= 0 AND entregada = 0 AND estado_reserva = 1 AND id_asesor = ". $usuario['idusuario']." ORDER BY año, id_mes";
			...
			if ($cant>0) {
				...
				$pdf->SetFont('');
				$pdf->Cell(0,5,utf8_decode($usuario['nombre']),0,1,'L');
				$pdf->Ln(2);
			}
...
$pdf->Output('Llegadas_por_llegar_mes_actual'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
