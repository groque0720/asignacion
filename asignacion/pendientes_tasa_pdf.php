...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('UNIDADES PENDINTES DE ENVIO - TASA'),0,0,'C');
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones_pendiente WHERE entregada = 0 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
...
$pdf->Output('Unidades_Pendientes_TASA'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
