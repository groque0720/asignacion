...
$grupos=mysqli_query($con, $SQL);?>
	...
	<div class="titulo-modelo">
		<?php echo "UNIDADES ASIGNADAS DE PLAN DE AHORRO"; ?>
	</div>
...
<?php while ($grupo=mysqli_fetch_array($grupos)) {?>
	...
	while ($modelo=mysqli_fetch_array($modelos)) {?>
		...
		$SQL="SELECT * FROM asignaciones WHERE borrar = 0 AND id_negocio = 2 AND entregada = 0 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
		$unidades = mysqli_query($con, $SQL);
		$cont_mod=0;
		while ($unidad=mysqli_fetch_array($unidades)) {$cont_mod++; ?>

		<?php 
			if ($cont_mod==1) { ?>
				<tr><td colspan="18"><div class=""></div></td></tr>	
				<tr><td colspan="18"><div class="">	<?php echo $grupo['grupo'].' '.$modelo['modelo'];?>	</div></td>	</tr>
				<tr><td colspan="18"><div class="">	</div></td>	</tr>
		<?php }	 ?>
