...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('PLAN AHORRO - LLEGADAS - PLANILLA DE ASIGNACIÓN'),0,0,'C');
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones_llegadas WHERE borrar=0 AND id_negocio = 2 AND entregada = 0 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
			...
			while ($unidad=mysqli_fetch_array($unidades)) {
				...
				$pdf->Cell(18,5,utf8_decode($unidad['nro_orden']),1,0,'L');
...
$pdf->Output('TPA_PlanilaAsignacion_'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
