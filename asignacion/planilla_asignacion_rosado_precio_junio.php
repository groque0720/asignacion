...
// Cabecera de página
	function Header()
	{
		if ($this->PageNo()==1) {
			...
			$this->Cell(150,5,('PLANILLA DE ASIGNACION - Rosadas Precio Junio'),0,0,'C');
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones_rosada_precio_junio WHERE borrar=0 AND entregada = 0 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
