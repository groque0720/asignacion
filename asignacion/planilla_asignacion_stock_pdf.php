...
$this->Cell(10,5,'Asesor',0,0,'C');
$this->Cell(19,5,'Costo',0,0,'C');
$this->Ln();
$this->Cell(0,0,'',1,0,'C');
$this->Ln(3);
}
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones_stock_pg WHERE pagado = 1 and id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
			...
			while ($unidad=mysqli_fetch_array($unidades)) {
				...
				$costo_total=$costo_total+round($unidad['costo']-$unidad['persepcion']);
				$cantidad_unidad = $cantidad_unidad + 1;
			}

			if ($cant>0) {
				$pdf->Cell(259,5,'',0,0,'C');
				$pdf->Cell(19,5,number_format($costo_modelo, 2, ',','.'),1,0,'C');
				$pdf->Ln(2);
			}
		...
		$pdf->Ln();
		$pdf->Cell(215,5,'',0,0,'C');
		$pdf->Cell(29,5,'Cantidad de Unidades ',0,0,'C');
		$pdf->Cell(5,5,$cantidad_unidad,0,0,'C');
		$pdf->Cell(10,5,'',0,0,'C');
		$pdf->Cell(19,5,number_format($costo_total, 2, ',','.'),1,0,'C');
		$pdf->Ln(2);

$pdf->Output('PlanilaAsignacion_Stock_'.cambiarFormatoFecha(date('Y-m-d')).'Hs'. strftime("%H:%M").'.pdf','I');
$pdf->close();
