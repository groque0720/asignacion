...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM view_asignaciones_stock WHERE id_ubicacion = 1 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
			...
			while ($unidad=mysqli_fetch_array($unidades)) {
				...
				$pdf->Cell(19,5,number_format($unidad['costo'], 2, ',','.'),1,0,'C');
				$pdf->Ln();
				$costo_modelo=$costo_modelo+$unidad['costo'];
				$costo_total=$costo_total+$unidad['costo'];
			}
