...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		$SQL="SELECT * FROM asignaciones WHERE borrar=0 AND id_negocio = 2 AND DATEDIFF(curdate(), fec_arribo) >= 60 AND entregada = 0 AND id_grupo = ". $grupo['idgrupo'];
		...
		while ($modelo=mysqli_fetch_array($modelos)) {
			...
			$SQL="SELECT * FROM asignaciones WHERE borrar=0 AND id_negocio = 2 AND DATEDIFF(curdate(), fec_arribo) >= 60 AND entregada = 0 AND id_modelo = ". $modelo['idmodelo'] ." ORDER BY año, id_mes, nro_orden, nro_unidad";
			$unidades = mysqli_query($con, $SQL);
