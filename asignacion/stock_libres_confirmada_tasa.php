...
class PDF extends FPDF
{
	...
	function Header()
	{
		if ($this->PageNo()==1) {
			...
			$this->Cell(100,5,('UNIDADES DISPONIBLES (CONFIRMADAS TASA + LIBRES)'),0,0,'C');
...
while ($grupo=mysqli_fetch_array($grupos)) {
	...
	while ($modelo=mysqli_fetch_array($modelos)) {
		...
		for ($i=0; $i < 9; $i++) {
			...
			$SQL="SELECT * FROM view_stock_tasa WHERE estado_tasa = 1 AND id_negocio = 1 AND  id_modelo =".$modelo['idmodelo']." AND id_mes = ".$m_a." AND año = ".$a_a;
			...
			if ($cant_stock>0) {
				...
				$stock_a[$i]['cant']=$stock['cantidad'] == 0 ? '-' : $stock['cantidad'];
			...
			if ($i==0) {
				// $SQL="SELECT sum(cantidad) AS cantidad FROM view_stock_libre_actuales_fisico_s_vender WHERE id_negocio = 1 AND id_modelo =".$modelo['idmodelo'];
				$SQL = "SELECT count(id_unidad) as cantidad FROM asignaciones WHERE id_negocio = 1 AND estado_tasa = 1 AND reservada = 0 AND  estado_reserva = 0 AND entregada = 0 AND  borrar = 0 AND id_modelo = ". $modelo['idmodelo']." AND ((id_mes <= $m_a AND año = $a_a) OR año < $a_a)";
				...
				if ($cant_stock_anterior>0) {
					...
					$stock_ant[$i]['cant']=$stock['cantidad'] == 0 ? '-' : $stock['cantidad'];
					// $stock_ant_g[$i]['cant']=$stock_ant_g[$i]['cant']+$stock_ant[$i]['cant'];
					$stock_ant_g[$i]['cant']=$stock_ant_g[$i]['cant']+$stock['cantidad'];
			...
			$SQL="SELECT * FROM view_stock_libre_actuales WHERE id_negocio = 1 AND id_modelo =".$modelo['idmodelo']." AND id_mes = ".$m_a." AND año = ".$a_a;
			...
			if (false) {
				...
				$reserva_a[$i]['cant']=$reserva['cantidad'] == 0 ? '-' : $reserva['cantidad'];
		...
		if ((int)$stock_a[0]['cant']+(int)$stock_ant[0]['cant']-(int)$reserva_a[0]['cant']!=0){
			...
			$s_p=(0+(int)$stock_ant[0]['cant']) == 0 ? '-' : (0+(int)$stock_ant[0]['cant']);
			$acum=0+(int)$stock_ant[0]['cant'];
		}else{
			$s_p='-';
		}
		...
		for ($i=1; $i <= 9; $i++) {
			...
			if ((int)$stock_a[$i]['cant']-(int)$reserva_a[$i]['cant']!=0 AND $i!=9){
				$s_p= isset($stock_ant[$i]['cant']) ? $stock_ant[$i]['cant'] : 0;
				$acum=$acum+$s_p;
			}else{
				$s_p='-';
			}
		...
		for ($i=0; $i < 9; $i++) {
			if ($i==0) {
				$total_r_m[$i]['cant']=(int)$total_r_m[$i]['cant']+(int)$stock_ant[0]['cant'];
				...
				$total_r_g[$i]['cant']=(int)$total_r_g[$i]['cant']+(int)$stock_ant[0]['cant'];
				$total_r_m[9]['cant']=(int)$total_r_m[9]['cant'] + (int)$total_r_m[$i]['cant'];

			}else{
				$total_r_m[$i]['cant']=(int)$total_r_m[$i]['cant'] + (int)$stock_ant[0]['cant'];
				$total_r_g[$i]['cant']=(int)$total_r_g[$i]['cant'] + 0;
				$total_r_m[9]['cant']=(int)$total_r_m[9]['cant'] + (int)$total_r_m[$i]['cant'];
