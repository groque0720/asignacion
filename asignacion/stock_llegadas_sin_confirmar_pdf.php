...
class PDF extends FPDF
{
	...
	function Header()
	{
		if ($this->PageNo()==1) {
			...
			$this->Cell(100,5,utf8_decode('UNIDADES STOCK FISICO SIN VENDER'),0,0,'C');
	...
	while ($modelo=mysqli_fetch_array($modelos)) {
		...
		for ($i=0; $i < 9; $i++) { 
			if ($i==0) {
				...
				$total_r_m[$i]['cant']=(int)$total_r_m[$i]['cant'] + (int)$stock_ant[i]['cant'];
