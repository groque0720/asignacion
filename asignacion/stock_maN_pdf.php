...
class PDF extends FPDF
{
	...
	function Header()
	{
		if ($this->PageNo()==1) {
			...
			$this->Cell(100,5,utf8_decode('PLANILLA DE STOCK GRAL (0 KM)'),0,0,'C');
...
while ($grupo=mysqli_fetch_array($grupos)) {
	...
	while ($modelo=mysqli_fetch_array($modelos)) {
		...
		for ($i=0; $i < 9; $i++) {
			...
			if ($i==0) {
				$SQL="SELECT sum(cantidad) AS cantidad FROM view_stock_libre_anteriores WHERE id_negocio = 1 AND id_modelo =".$modelo['idmodelo'];
