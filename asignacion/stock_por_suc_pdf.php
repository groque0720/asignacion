...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('STOCK POR SUCURSAL'),0,0,'C');
...
$cant_total=0;
	while ($sucursal=mysqli_fetch_array($sucursales)) {
		...
		$pdf->Ln(2);
			...
			$SQL="SELECT * FROM view_asignaciones_llegadas WHERE id_ubicacion = ". $sucursal['idsucursal'];
			...
			$pdf->SetFont('Arial','B',10); 
			$pdf->SetFont('');
			$pdf->Cell(0,5,utf8_decode('Cantidad de unidades en sucursal '.$sucursal['sucursal'].': '.$cant_unidades.' Un.'),1,1,'L');
			$pdf->Ln(2);
	...
	$pdf->SetFont('Arial','B',10); 
	$pdf->SetFont('');
	$pdf->Cell(0,5,utf8_decode('Cantidad Total de unidades en Derka y Vargas S. A.: '.$cant_total.' Un.'),1,1,'L');
	$pdf->Ln(2);
