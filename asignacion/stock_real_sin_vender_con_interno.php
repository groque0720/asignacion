...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		for ($i=1; $i < 2 ; $i++) {
			...
			$this->Cell(60,5,'Stock - '.$mes_a[$m_a].' '.$a_a,1,0,'C');
			$m_a++;
		...
		$this->Cell(20,5,'con Interno',1,0,'C');
		//$this->Cell(0.8,5,'',0,0,'C');


		//$this->Cell(0.3,5,'',1,0,'C');
		$this->Cell(20,5,'stock',1,0,'C');
		$this->Cell(20,5,'Asig.',1,0,'C');
...
$cant_grupo=0;
$total_pisadas_gral = 0;
$total_conInterno_gral = 0;
...
while ($grupo=mysqli_fetch_array($grupos)) {
	...
	$total_pisadas_grupo = 0;
	$total_conInterno_grupo = 0;
	...
	while ($modelo=mysqli_fetch_array($modelos)) {
		...
		$SQL="SELECT * FROM view_asignacion_pisadas WHERE id_modelo = ". $modelo['idmodelo'];
		$pisadas=mysqli_query($con, $SQL);
		$cant_pisada=mysqli_num_rows($pisadas);

		$total_pisadas_grupo = $total_pisadas_grupo + $cant_pisada;
		$total_pisadas_gral = $total_pisadas_gral + $cant_pisada;


		$SQL="SELECT * FROM view_stock_libre_con_interno_sin_vender WHERE id_modelo = ". $modelo['idmodelo'];
		$conInterno=mysqli_query($con, $SQL);
		$cant_conInterno=mysqli_num_rows($conInterno);

		$total_conInterno_grupo = $total_conInterno_grupo + $cant_conInterno;
		$total_conInterno_gral = $total_conInterno_gral + $cant_conInterno;
		...
		for ($i=0; $i < 1; $i++) {
			...
			if ($cant_stock>0) {
				...
				$stock_a[6]['cant']=(int)$stock_a[6]['cant']+(int)$stock['cantidad'];
			}else{
				$stock_a[$i]['cant']=0;
			}
			...
			if ($i==0) {
				...
				if ($cant_stock_anterior>0) {
					...
					$reserva_a[6]['cant']=(int)$stock['cantidad'];
				}else{
					$stock_ant[$i]['cant']=0;
				}
			}
			...
			if ($cant_stock>0) {
				...
				$reserva_a[6]['cant']=(int)$reserva_a[6]['cant']+(int)$reserva['cantidad'];
			}else{
				$reserva_a[$i]['cant']=0;
			}
		...
		$pdf->Cell(40,5,'   '.$modelo['modelo'],1,0);
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$cant_pisada,1,0,'C');
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$cant_conInterno,1,0,'C');
		//$pdf->Cell(0.8,5,'',0,0,'C');

		$acum=0;
		if ((int)$stock_a[0]['cant']+(int)$stock_ant[0]['cant']-(int)$reserva_a[0]['cant']!=0){
			$s_p=(int)$stock_a[0]['cant']+(int)$stock_ant[0]['cant']-(int)$reserva_a[0]['cant'];
			$acum=(int)$stock_a[0]['cant']+(int)$stock_ant[0]['cant']-(int)$reserva_a[0]['cant'];
		}else{
			$s_p='-';
		}
		...
		$pdf->Cell(20,5,$s_p,1,0,'C');
		$pdf->Cell(20,5,$s_a,1,0,'C');
		//Relleno Linea Con datos traidos por modelos.-

		for ($i=1; $i < 1; $i++) {

			if ((int)$stock_a[$i]['cant']-(int)$reserva_a[$i]['cant']!=0 AND $i!=7){
		...
		for ($i=0; $i < 1; $i++) {
			if ($i==0) {
				...
				$total_r_m[7]['cant']=(int)$total_r_m[7]['cant'] + (int)$total_r_m[$i]['cant'];
	...
	$total_r_g[7]['cant']=$total_r_g[7]['cant']+$total_r_m[7]['cant'];


		$pdf->SetFont('Arial','BI',6.5);
		$pdf->Cell(40,5,'TOTAL '.$grupo['grupo'],1,0);
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$total_pisadas_grupo,1,0,'C');
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$total_conInterno_grupo,1,0,'C');
		//$pdf->Cell(0.8,5,'',0,0,'C');
		...
		//$pdf->Cell(0.3,5,'',1,0,'C');//linea separador de meses
		$pdf->SetDrawColor(184, 184, 184);
		$pdf->Cell(20,5,$total_r_m[$i]['cant'],1,0,'C');
		$pdf->Cell(20,5,$total_m[$i]['cant'],1,0,'C');
		}
		$pdf->SetDrawColor(60, 60, 60);
		$pdf->Cell(0.3,5,'',1,0,'C');//linea separador de meses
		$pdf->SetDrawColor(184, 184, 184);
	...
	if ($cant_grupo==2) {
		$cant_grupo++;
		$pdf->Cell(40,5,'TOTAL HILUX',1,0);
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$total_pisadas_gral,1,0,'C');
		$pdf->Cell(0.8,5,'',0,0,'C');
		$pdf->Cell(20,5,$total_conInterno_gral,1,0,'C');
		//$pdf->Cell(0.8,5,'',0,0,'C');
		...
		//$pdf->Cell(0.3,5,'',1,0,'C');//linea separador de meses
		$pdf->SetDrawColor(184, 184, 184);
		$pdf->Cell(20,5,$total_r_g[$i]['cant'],1,0,'C');
		$pdf->Cell(20,5,$total_g[$i]['cant'],1,0,'C');
		}
		$pdf->SetDrawColor(60, 60, 60);
		$pdf->Cell(0.3,5,'',1,0,'C');//linea separador de meses
		$pdf->SetDrawColor(184, 184, 184);
