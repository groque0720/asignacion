...
class PDF extends FPDF
{
	...
	function Header()
	{
		if ($this->PageNo()==1) {
			...
			$this->Cell(100,5,('PLANILLA DE STOCK - A PARTIR DEL MES SIGUIENTE'),0,0,'C');
		...
		$this->Ln();
		$this->Cell(30,5,'Modelos',1,0,'C');
		$this->Cell(0.8,5,'',0,0,'C');

		$this->Cell(6,5,'S/C',1,0,'C');
		$this->Cell(0.8,5,'',0,0,'C');
...
while ($grupo=mysqli_fetch_array($grupos)) {
	...
	while ($modelo=mysqli_fetch_array($modelos)) {
		...
		$SQL="select `asignaciones`.`id_unidad` AS `id_unidad`,`asignaciones`.`id_modelo` AS `id_modelo` from `asignaciones` where ((`asignaciones`.`reservada` = 1) and (`asignaciones`.`estado_reserva` = 0) and (`asignaciones`.`entregada` = 0) and (`asignaciones`.`borrar` = 0)) and id_modelo = ". $modelo['idmodelo'];
	...
	$total_r_g[9]['cant']=$total_r_g[9]['cant']+$total_r_m[9]['cant'];
		...
		$pdf->Cell(30,5,'TOTAL '.$grupo['grupo'],1,0);
		$pdf->Cell(0.8,5,'',0,0,'C');
