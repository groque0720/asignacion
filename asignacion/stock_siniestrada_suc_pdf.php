...
function Header()
{
	if ($this->PageNo()==1) {
		...
		$this->Cell(150,5,utf8_decode('UNIDADES SINIESTRADAS POR SUCURSAL'),0,0,'C');
...
$cant_total=0;
	while ($sucursal=mysqli_fetch_array($sucursales)) {
		...
		$pdf->Ln(2);
			...
			$SQL="SELECT * FROM view_asignaciones_siniestrada WHERE id_ubicacion = ". $sucursal['idsucursal'];
