...
$SQL="SELECT * FROM asignaciones_usados_estados_certificados ORDER BY posicion";
...
while ($estado=mysqli_fetch_array($estado_usado)) {
	...
	$SQL="SELECT *, DATEDIFF(DATE(NOW()),fec_recepcion)as ant FROM asignaciones_usados WHERE entregado = 0 AND id_estado = 1 AND id_estado_certificado =".$estado['id']." ORDER BY vehiculo";
