...
if ($_SESSION["es_gerente"]==1) {
	...
	$SQL="INSERT INTO prospectos_clientes (fecha_alta, id_usuario) VALUES ('".date("Y-m-d")."',{$id_usuario}) ";
	mysqli_query($con, $SQL);
}
...
<div class="formulario">
	<form class="form-formulario" action="" method="POST">
		...
		<input type="hidden" name='es_dato' id="es_dato" value="1">
...
<!-- 				<div class="form-linea ">
					...
					<?php
						...
						if ($_SESSION["es_gerente"]==1) { ?>
							<div class="ancho-35 derecha-texto">
								...
								<select class="form-inputs ancho-70 id_usuario_dato" name="id_usuario" id="id_usuario">
								<?php 
								...
								<?php while ($asesor=mysqli_fetch_array($asesores)) { ?>
									<option value="<?php echo $asesor['idusuario'];?>" <?php if ($asesor['idusuario']==$cliente['id_usuario']) {
										echo 'selected'; }?>><?php echo $asesor['nombre']; ?></option>
								<?php	} ?>
								</select>
							</div>
						<?php }else{ ?>
							...
							<div class="ancho-30 derecha-texto">
								...
								<?php 
									...
									while ($estado = mysqli_fetch_array($estados)) { ?>
										<option value="<?php echo $estado['id']; ?>" <?php if ($estado['id']==$cliente['id_estado_cliente']) { echo 'selected';} ?><?php echo $mostrar; ?>><?php echo $estado['estado_cliente']; ?></option>
								<?php }  ?>
				...
				<div class="form-linea">
					...
					<div class="ancho-45 centrar-texto">
						...
						<option value="0"></option>
						<?php
							$SQL="SELECT * FROM localidades WHERE id_provincia =".$cliente['id_provincia'];
...
<script src="js/dato_formulario.js"></script>