...
$("#id_modelo").change(function(event) {
	id_modelo=$(this).val();
	$(".mod").show();
	$.ajax({
		url:"ajax_actualizar_modelo.php",
		cache:false,
		type:"POST",
		data:{id_modelo:id_modelo},
		success:function(result){
				$(".mod").hide();
				$("#id_version").html(result);
			}
	});
});
...
$(".form-formulario").submit(function(event) {
	...
	if ($("#id_modelo_tpa").val()==0 && band==0) {
		$("#id_modelo_tpa").focus();
		swal("Campo Obligatorio!", "Por favor Ingrese 'Modelo Plan de Ahorro' para guardar");
		band=1;
	}

	if ($("#id_modo_acercamiento").val()==0 && band==0) {
		$("#id_modo_acercamiento").focus();
		swal("Campo Obligatorio!", "Por favor Ingrese 'Modo de Acercamiento' para guardar");
		band=1;
	}

	if ($("#id_canal_acercamiento").val()==0 && band==0) {
		$("#id_canal_acercamiento").focus();
		swal("Campo Obligatorio!", "Por favor Ingrese 'Canal de Acercamiento' para guardar");
		band=1;
	}

	if ($("#id_ponderacion").val()==0 && band==0) {
		$("#id_ponderacion").focus();
		swal("Campo Obligatorio!", "Por favor Ingrese 'Ponderación' para guardar");
		band=1;
	}
