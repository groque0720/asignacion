...
if ($_SESSION["es_gerente"]==1) {
	...
	$SQL="SELECT * FROM prospectos WHERE guardado = 1 ORDER BY fecha_carga DESC LIMIT $inicio, $final";
	$prospectos=mysqli_query($con, $SQL);

}else {

	$SQL="SELECT * FROM prospectos WHERE guardado = 1 AND id_usuario = {$_SESSION["id"]} ORDER BY fecha_carga DESC LIMIT $inicio, $final";
	$prospectos=mysqli_query($con, $SQL);
}
