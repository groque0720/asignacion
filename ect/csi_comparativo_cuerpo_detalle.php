<div class="ancho-100 ">
	...
	<?php 
		if ($cant_meses <= 6) {
			$class='ancho-85 margen-arriba-5';
		}else{
			if ($cant_meses <= 8) {
			$class='ancho-95 margen-arriba-5';
			}else{
						...
						<tr style="<?php echo $estilo_fila; ?>" >
							...
							<?php for ($i=0; $i < $cant_meses; $i++) { 
								...
								if ($asesor==0) {
									...
									$numero=mysqli_num_rows($csi_sucs);

									if ($numero==0) { $csi_suc_det['puntaje']=0;

									$csi_mes_a[$i]['csi']=0;
									$csi_mes_a[$i]['xy']=0*($i+1);
									...
									$csi_suc_det=mysqli_fetch_array($csi_suc_dets);
										...
										$csi_suc_dets=mysqli_query($con, $SQL);
										if (mysqli_num_rows($csi_suc_dets)) {$csi_suc_det=mysqli_fetch_array($csi_suc_dets);
											}else{$csi_suc_det=mysqli_fetch_array($csi_suc_dets);
										$csi_suc_det['puntaje']=0;}
						...
						<td>CSI TOTAL  MES </td>
							...
							<?php for ($i=0; $i < $cant_meses; $i++) { ?>
								<td class="centrar-texto"><?php 
								 ...
								 $item_gral_acum = $item_gral_acum + $csi_mes_a[$i]['csi'];  ?></td>
...
<div id="grafico_comparativo_csi" class="<?php echo $class; ?>"></div>
 <script>
 	
 	Highcharts.chart('grafico_comparativo_csi', {
 ...
 plotOptions: {
    ...
    series: [{
        ...
        data: [
        		<?php
        	  	 	for ($i=0; $i < $cant_meses; $i++) { 
					echo number_format($csi_mes_a[$i]['csi'],2).",";
					}
				?>
			]
    }, {
        ...
        data: [
				<?php
        	  	 	for ($i=0; $i < $cant_meses; $i++) { 
					echo number_format($csi_mes_a[$i]['tendencia'],2).",";
					}
				?>
         	]
    }]
