...
while ($item=mysqli_fetch_array($items)) {
	...
	$SQL="SELECT id FROM ect_csi_sucursales_dyv_detalle WHERE id_csi_sucursal= $id AND id_item_dyv = ".$item['id'];
	...
	if ($cant == 0 ) {
			...
			$SQL="INSERT INTO ect_csi_sucursales_dyv_detalle (id_csi_sucursal, id_item_dyv) VALUES ( $id,".$item['id'].")";
...
<div class="ancho-50 centrar-texto centrar-caja negrita margen-arriba-10 fz-15">
	<?php echo 'Sucursal: '.$sucursal.' -  Detalle CSI DYV'?>
</div>
...
<table class="ancho-50 margen-arriba-10">
	...
	<?php 
		...
		$SQL="SELECT * FROM ect_csi_sucursales_dyv_detalle WHERE id_csi_sucursal= $id";
		$csi_detalles=mysqli_query($con, $SQL);
		$fila=0;
...
<script src="js/csi_sucursales_cuerpo_dyv_detalle.js"></script>