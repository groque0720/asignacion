...
if ($id_pregunta > 0) {
	...
	if (mysqli_num_rows($res_p) == 0) { header("Location: preguntas.php?id_encuesta=$id_encuesta"); exit(); }
...
<div class="enc-subpag-cabecera">
	<a class="btn-enc btn-enc-gris btn-enc-sm" href="preguntas.php?id_encuesta=<?php echo $id_encuesta; ?>">← Volver</a>
...
<form id="frm_pregunta">
	...
	<div class="enc-form-row">
		...
		<div>
			...
			<p style="color:#888;font-size:10px;margin-top:3px;">
				No hay áreas definidas. <a href="areas.php" target="_blank">Crear áreas</a>.
			</p>
			<?php endif; ?>
		</div>
	</div>
	...
	<div class="enc-form-acciones" style="margin-top:18px;">
		...
		<a href="preguntas.php?id_encuesta=<?php echo $id_encuesta; ?>" class="btn-enc btn-enc-gris">
...
$(function(){
			...
			success: function(resp){
				...
				if (resp.indexOf("ok:") === 0) {
					...
					if (id_pregunta_actual == 0) {
						...
						window.location = "pregunta_form.php?id_encuesta=" + id_encuesta_actual + "&id=" + nuevo_id;
					} else {
						window.location = "preguntas.php?id_encuesta=" + id_encuesta_actual;
					}
				} else {
	...
	$(document).on("click", ".btn-del-opcion", function(){
		...
		swal({
			...
			mostrarCargando();
			$.post("../ajax/opcion_baja.php", {id_opcion: id_op}, function(r){
				ocultarCargando();
				if (r=="ok") $item.remove();
