...
if ($res === 'encuesta') {
    if ($accion === 'crear' || $accion === 'editar') {
        ...
        if ($accion === 'crear') {
            if (!mysqli_query($con, "INSERT INTO enc_encuestas (nombre, descripcion, mensaje_bienvenida, activa)
                                     VALUES ('$n', '$d', '$mb', 0)")) fail(mysqli_error($con));
            ok(['id_encuesta' => mysqli_insert_id($con)]);
        } else {
            $id = (int)P('id_encuesta');
            if (!mysqli_query($con, "UPDATE enc_encuestas SET nombre='$n', descripcion='$d', mensaje_bienvenida='$mb'
    ...
    if ($accion === 'activar') {
        $id = (int)P('id_encuesta');
        mysqli_query($con, "UPDATE enc_encuestas SET activa = 0");          // sólo una activa
        mysqli_query($con, "UPDATE enc_encuestas SET activa = 1 WHERE id_encuesta = $id");
        ok();
    }
    if ($accion === 'desactivar') {
        $id = (int)P('id_encuesta');
        mysqli_query($con, "UPDATE enc_encuestas SET activa = 0 WHERE id_encuesta = $id");
        ok();
    }
    if ($accion === 'baja') {
        $id = (int)P('id_encuesta');
        $a = mysqli_fetch_assoc(mysqli_query($con, "SELECT activa FROM enc_encuestas WHERE id_encuesta=$id"));
        if ($a && (int)$a['activa'] === 1) fail('No se puede eliminar una encuesta activa. Desactivala primero.');
        mysqli_query($con, "UPDATE enc_encuestas SET baja = 1 WHERE id_encuesta = $id");
        ok();
    }
...
if ($res === 'pregunta') {
    if ($accion === 'guardar') {
        ...
        if ($id > 0) {
            if (!mysqli_query($con,
                "UPDATE enc_preguntas SET texto_pregunta='$t', tipo_pregunta=$tipo, pondera=$pondera,
            ...
            $ord = (int)mysqli_fetch_assoc(mysqli_query($con,
                "SELECT COALESCE(MAX(nro_orden),0)+1 n FROM enc_preguntas WHERE id_encuesta=$idE"))['n'];
            if (!mysqli_query($con,
                "INSERT INTO enc_preguntas (id_encuesta, nro_orden, texto_pregunta, tipo_pregunta, pondera, es_observacion, id_area, cond_id_preg_ref, cond_operador, cond_valor)
        ...
        if (in_array($tipo, [3, 4], true)) {
            ...
            foreach ($opciones as $o) {
                ...
                if ($oid > 0) {
                    mysqli_query($con, "UPDATE enc_opciones SET texto_opcion='$te', nro_orden=$n, baja=0 WHERE id_opcion=$oid AND id_pregunta=$id");
                    $idsConservar[] = $oid;
                } else {
                    mysqli_query($con, "INSERT INTO enc_opciones (id_pregunta, texto_opcion, nro_orden) VALUES ($id, '$te', $n)");
            ...
            mysqli_query($con, "UPDATE enc_opciones SET baja=1 WHERE id_pregunta=$id AND id_opcion NOT IN ($keep)");
        }
        ok(['id_pregunta' => $id]);
    }
    ...
    if ($accion === 'baja') {
        ...
        $dep = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) n FROM enc_preguntas WHERE cond_id_preg_ref=$id AND baja=0"));
        if ($dep && (int)$dep['n'] > 0) fail('Otra pregunta depende de esta como condición. Quitá esa condición primero.');
        mysqli_query($con, "UPDATE enc_preguntas SET baja=1 WHERE id_pregunta=$id");
        ok();
    }
    ...
    if ($accion === 'orden') {
        $id  = (int)P('id_pregunta');
        $dir = P('dir'); // 'up' | 'down'
        $cur = mysqli_fetch_assoc(mysqli_query($con, "SELECT id_encuesta, nro_orden FROM enc_preguntas WHERE id_pregunta=$id"));
        ...
        $vec = mysqli_fetch_assoc(mysqli_query($con, "SELECT id_pregunta, nro_orden FROM enc_preguntas WHERE id_encuesta=$idE AND baja=0 AND $cmp LIMIT 1"));
        if ($vec) {
            $vid = (int)$vec['id_pregunta']; $vord = (int)$vec['nro_orden'];
            mysqli_query($con, "UPDATE enc_preguntas SET nro_orden=$vord WHERE id_pregunta=$id");
            mysqli_query($con, "UPDATE enc_preguntas SET nro_orden=$ord WHERE id_pregunta=$vid");
        }
        ok();
    }
    fail('Acción inválida');
...
if ($res === 'area') {
    if ($accion === 'guardar') {
        ...
        $ord = (int)P('nro_orden');
        if ($id > 0) {
            if (!mysqli_query($con, "UPDATE enc_areas SET nombre='$n', color='$col', nro_orden=$ord WHERE id_area=$id")) fail(mysqli_error($con));
        } else {
            if (!mysqli_query($con, "INSERT INTO enc_areas (nombre, color, nro_orden) VALUES ('$n', '$col', $ord)")) fail(mysqli_error($con));
            $id = mysqli_insert_id($con);
        }
        ok(['id_area' => $id]);
    }
    if ($accion === 'eliminar') {
        $id = (int)P('id_area');
        mysqli_query($con, "UPDATE enc_preguntas SET id_area=NULL WHERE id_area=$id");  // liberar FK
        mysqli_query($con, "DELETE FROM enc_areas WHERE id_area=$id");
        ok();
    }
    fail('Acción inválida');
}
...
if ($res === 'nivel') {
    if ($accion === 'guardar') {
        ...
        if ($id > 0) {
            if (!mysqli_query($con, "UPDATE enc_niveles SET nombre='$n', valor_desde=$de, valor_hasta=$ha, color='$col' WHERE id_nivel=$id")) fail(mysqli_error($con));
        } else {
            if (!mysqli_query($con, "INSERT INTO enc_niveles (nombre, valor_desde, valor_hasta, color) VALUES ('$n', $de, $ha, '$col')")) fail(mysqli_error($con));
            $id = mysqli_insert_id($con);
        }
        ok(['id_nivel' => $id]);
    }
    if ($accion === 'eliminar') {
        $id = (int)P('id_nivel');
        mysqli_query($con, "DELETE FROM enc_niveles WHERE id_nivel=$id");
        ok();
    }
    fail('Acción inválida');
}
