...
while ($s = mysqli_fetch_assoc($rs)) $sucursales[] = ['id' => (int)$s['idsucursal'], 'nombre' => enc_utf8($s['sucursal'])];
...
$title    = 'Resultados · 0km';
$bodyData = 'dashboardCero(' . $sucursalesJson . ')';
$bodyInit = 'init()';
$jsFile   = 'dashboard.js';
  ...
  <header class="bg-slate-900 text-white shadow-lg sticky top-0 z-30">
    <div class="max-w-[1400px] mx-auto px-6 py-3 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div><h1 class="text-sm font-bold leading-tight">Resultados de Encuestas · 0km</h1>
  ...
  <main class="max-w-[1400px] mx-auto px-6 py-5 space-y-5">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 items-end"
        ...
        <!-- Fila 2 -->
        <div>
          <label class="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Grupo</label>
          <select x-model.number="filtros.grupo" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
            <option :value="0">Todos</option>
            <template x-for="g in opciones.grupos" :key="g.id"><option :value="g.id" x-text="g.nombre"></option></template>
          </select>
        </div>
        <div>
          <label class="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Versión</label>
          <select x-model.number="filtros.modelo" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
            <option :value="0">Todas</option>
            <template x-for="md in opciones.modelos" :key="md.id"><option :value="md.id" x-text="md.nombre"></option></template>
          </select>
        </div>
        <div>
        ...
        <div class="col-span-2 sm:col-span-3 flex items-center gap-2">
      ...
      <div class="overflow-x-auto" style="max-height: calc(100vh - 320px);">
        <table class="w-full text-sm dv-table">
          <thead><tr>
            ...
            <th class="px-3 py-2.5 text-left font-semibold">Chasis</th>
          ...
          <tbody>
            <template x-for="r in tabla" :key="r.id_respuesta">
              <tr class="hover:bg-blue-50/40">
                ...
                <td class="px-3 py-2 text-slate-500 font-mono text-[11px] uppercase" x-text="r.chasis || '—'"></td>
            ...
            <tr x-show="!loading && tabla.length===0"><td colspan="8" class="px-3 py-10 text-center text-slate-400"><i class="fas fa-inbox text-2xl mb-2 block"></i> Sin respuestas para este filtro.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
