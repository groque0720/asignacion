<?php
/* Resultados · Detalle de una respuesta de encuesta 0km. */
...
$unidad = mysqli_fetch_assoc(mysqli_query($con,
    "SELECT a.cliente, a.chasis, a.nro_orden, a.fec_entrega, a.id_sucursal,
     ...
     WHERE a.id_unidad = $id_unidad LIMIT 1"));

$vehiculo = $unidad ? trim(trim((string)$unidad['grupo']) . ' ' . trim((string)$unidad['modelo'])) : '';
...
$title = 'Resultado · ' . ($unidad ? enc_utf8($unidad['cliente']) : 'Encuesta');
$extraHead = '';
?>
<!doctype html>
<html lang="es">
  ...
  <header class="bg-slate-900 text-white shadow-lg sticky top-0 z-30">
    <div class="max-w-[1000px] mx-auto px-6 py-3 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <p class="text-slate-400 text-xs">0km · Derka y Vargas S.A.</p></div>
      </div>
  ...
  <main class="max-w-[1000px] mx-auto px-6 py-6 space-y-5">
    ...
    <div class="grid md:grid-cols-3 gap-5">
      <div class="md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <h2 class="text-lg font-bold text-slate-900"><?= htmlspecialchars(enc_utf8($unidad['cliente'] ?? '—')) ?></h2>
        <p class="text-slate-600"><?= htmlspecialchars(enc_utf8($vehiculo)) ?></p>
        <div class="grid grid-cols-2 gap-3 mt-4 text-sm">
          <div><span class="text-slate-400">Chasis</span><br><span class="font-medium font-mono uppercase"><?= htmlspecialchars($unidad['chasis'] ?? '—') ?: '—' ?></span></div>
          <div><span class="text-slate-400">N° Orden</span><br><span class="font-medium font-mono"><?= htmlspecialchars($unidad['nro_orden'] ?? '—') ?: '—' ?></span></div>
          <div><span class="text-slate-400">Asesor</span><br><span class="font-medium"><?= htmlspecialchars(enc_utf8($unidad['asesor'] ?? '—')) ?></span></div>
          <div><span class="text-slate-400">Sucursal</span><br><span class="font-medium"><?= htmlspecialchars(enc_utf8($unidad['sucursal'] ?? '—')) ?></span></div>
          <div><span class="text-slate-400">Entrega</span><br><span class="font-medium"><?= fmtFecha($unidad['fec_entrega'] ?? '') ?></span></div>
          <div><span class="text-slate-400">Encuesta</span><br><span class="font-medium"><?= htmlspecialchars(enc_utf8($resp['encuesta_nombre'])) ?></span></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <?php foreach ($areaAgg as $nombre => $a): $pa = $a['n'] ? $a['suma']/$a['n'] : 0; $nv = enc_nivel($pa); ?>
        <div class="rounded-lg border border-gray-100 p-3">
          <div class="flex items-center gap-1.5 mb-1">
            <span class="w-2.5 h-2.5 rounded-full" style="background:<?= $a['color'] ?: '#607d8b' ?>"></span>
            <span class="text-xs text-slate-500 truncate"><?= htmlspecialchars(enc_utf8($nombre)) ?></span>
          </div>
          <div class="flex items-center justify-between">
            <p class="text-xl font-bold num text-slate-900"><?= number_format($pa,1) ?></p>
            <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold text-white" style="background:<?= $nv['color'] ?>"><?= htmlspecialchars(enc_utf8($nv['nombre'])) ?></span>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
    ...
    <div class="space-y-3">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div class="flex items-center gap-2 flex-wrap mb-1.5">
          ...
          <?php if ($p['area_nombre']): ?><span class="text-[11px] font-semibold px-2 py-0.5 rounded text-white" style="background:<?= $p['area_color'] ?: '#607d8b' ?>"><?= htmlspecialchars(enc_utf8($p['area_nombre'])) ?></span><?php endif; ?>
        </div>
        <p class="font-medium text-slate-900 mb-2"><?= htmlspecialchars(enc_utf8($p['texto_pregunta'])) ?></p>
        ...
        <?php elseif ($tipo === 3): ?>
          <div class="flex flex-wrap gap-1.5">
            <?php foreach (($opcsPorPreg[(int)$p['id_pregunta']] ?? []) as $o): ?>
              <span class="text-xs px-2 py-1 rounded bg-blue-50 text-blue-700"><i class="fas fa-check mr-1"></i><?= htmlspecialchars(enc_utf8($o['texto_opcion'])) ?></span>
            <?php endforeach; ?>
          ...
          <div class="space-y-1">
            <?php foreach (($opcsPorPreg[(int)$p['id_pregunta']] ?? []) as $o): $ok=(int)$o['valor_elegido']===1; ?>
              <div class="flex items-center justify-between text-sm border-b border-gray-50 py-1">
                <span class="text-slate-600"><?= htmlspecialchars(enc_utf8($o['texto_opcion'])) ?></span>
        ...
        <?php elseif ($tipo === 5): ?>
          ...
          <p class="text-sm text-slate-700 bg-slate-50 rounded-lg p-3 <?= $txt==='' ? 'text-slate-400 italic' : '' ?>"><?= $txt!=='' ? nl2br(htmlspecialchars(enc_utf8($txt))) : 'Sin comentarios' ?></p>
        <?php endif; ?>
      </div>
      <?php endforeach; ?>
