...
<div class="enc-sec-header">
	...
	<a class="btn-enc btn-enc-gris btn-enc-sm" href="admin/areas.php" style="margin-left:auto;">
		<span class="icon-tags"></span> Áreas
	</a>
	<a class="btn-enc btn-enc-gris btn-enc-sm" href="admin/niveles.php" style="margin-left:6px;">
		...
		<tr id="enc-fila-<?php echo $enc['id_encuesta']; ?>">
			...
			<td class="celda-acciones">
				...
				<a class="btn-enc btn-enc-azul btn-enc-sm"
				   href="admin/preguntas.php?id_encuesta=<?php echo $enc['id_encuesta']; ?>">
...
$(function(){
	...
	$(document).on("click", ".btn-activar", function(){
		...
		swal({
			...
			mostrarCargando();
			$.post("ajax/encuesta_activar.php", {id_encuesta: id}, function(resp){
				ocultarCargando();
	...
	$(document).on("click", ".btn-baja-enc", function(){
		...
		swal({
			...
			mostrarCargando();
			$.post("ajax/encuesta_baja.php", {id_encuesta: id}, function(resp){
				ocultarCargando();
				if (resp == "ok") { location.reload(); }
				else { swal("Error", resp, "error"); }
			});
