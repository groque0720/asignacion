          ...
          <tbody>
            ...
            <template x-for="r in (loading ? [] : rows)" :key="r.id_unidad">
              <tr class="transition-colors hover:bg-blue-50/40">
                ...
                <td class="px-3 py-2">
                  <div class="truncate text-slate-700" x-text="r.grupo" :title="r.grupo"></div>
                  <div x-show="r.modelo" class="text-[11px] text-slate-400 truncate" x-text="r.modelo" :title="r.modelo"></div>
                </td>
                <td class="px-3 py-2 whitespace-nowrap">
                  <span x-show="r.chasis" class="font-mono text-[11px] text-slate-600 uppercase" x-text="r.chasis"></span>
                  <span x-show="!r.chasis" class="text-slate-300">—</span>
                </td>
                <td class="px-3 py-2 whitespace-nowrap">
                  <span x-show="r.nro_orden" class="font-mono text-[11px] text-slate-600" x-text="r.nro_orden"></span>
                  <span x-show="!r.nro_orden" class="text-slate-300">—</span>
                </td>
                <td class="px-3 py-2">
                  <div class="truncate text-slate-600" x-text="r.asesor" :title="r.asesor"></div>
                </td>
                <td class="px-3 py-2 whitespace-nowrap text-slate-600" x-text="r.sucursal || ('Suc. ' + r.id_sucursal)"></td>
