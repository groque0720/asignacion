...
if ($token === '' || $id_token <= 0) {
	header("Location: ../publico/expirada.php"); exit();
}
...
if (mysqli_num_rows($res_tok) == 0) {
	header("Location: ../publico/expirada.php?tipo=completada"); exit();
}
...
if (!is_array($respuestas) || empty($respuestas)) {
	header("Location: ../publico/responder.php?t=" . urlencode($token) . "&error=empty"); exit();
}
...
if (!mysqli_query($con, $SQL_resp)) {
	...
	header("Location: ../publico/gracias.php"); exit();
}
...
header("Location: ../publico/gracias.php");
exit();
?>
