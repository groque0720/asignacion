...
if ($res === 'encuestas') {
    $q = mysqli_query($con,
        "SELECT e.id_encuesta, e.nombre, e.descripcion, e.mensaje_bienvenida, e.activa, e.fecha_creacion,
                (SELECT COUNT(*) FROM encu_preguntas p WHERE p.id_encuesta = e.id_encuesta AND p.baja = 0) AS nro_preguntas
         FROM encu_encuestas e WHERE e.baja = 0
         ORDER BY e.activa DESC, e.id_encuesta DESC");
    $items = [];
    while ($r = mysqli_fetch_assoc($q)) {
        $items[] = [
            'id_encuesta'        => (int)$r['id_encuesta'],
            'nombre'             => eu_utf8($r['nombre']),
            'descripcion'        => eu_utf8($r['descripcion']),
            'mensaje_bienvenida' => eu_utf8($r['mensaje_bienvenida']),
            'activa'             => (int)$r['activa'],
    ...
    $q = mysqli_query($con, "SELECT id_area, nombre, color, nro_orden FROM encu_areas ORDER BY nro_orden ASC, nombre ASC");
    $items = [];
    while ($r = mysqli_fetch_assoc($q)) {
        $items[] = ['id_area' => (int)$r['id_area'], 'nombre' => eu_utf8($r['nombre']),
                    'color' => $r['color'], 'nro_orden' => (int)$r['nro_orden']];
    }
    $out['items'] = $items;

} elseif ($res === 'niveles') {
    $q = mysqli_query($con, "SELECT id_nivel, nombre, valor_desde, valor_hasta, color FROM encu_niveles ORDER BY valor_desde DESC");
    $items = [];
    while ($r = mysqli_fetch_assoc($q)) {
        $items[] = ['id_nivel' => (int)$r['id_nivel'], 'nombre' => eu_utf8($r['nombre']),
    ...
    $enc = mysqli_fetch_assoc(mysqli_query($con,
        "SELECT id_encuesta, nombre, activa FROM encu_encuestas WHERE id_encuesta = $idE AND baja = 0 LIMIT 1"));
    if (!$enc) { echo json_encode(['error' => 'Encuesta inexistente']); exit(); }

    // Áreas (para el select del form)
    $areas = [];
    $qa = mysqli_query($con, "SELECT id_area, nombre, color FROM encu_areas ORDER BY nro_orden ASC");
    while ($a = mysqli_fetch_assoc($qa)) $areas[] = ['id_area' => (int)$a['id_area'], 'nombre' => eu_utf8($a['nombre']), 'color' => $a['color']];
    ...
    $q = mysqli_query($con,
        "SELECT p.*, a.nombre AS area_nombre, a.color AS area_color, ref.texto_pregunta AS ref_texto
         ...
         LEFT JOIN encu_preguntas ref ON ref.id_pregunta = p.cond_id_preg_ref
    ...
    while ($r = mysqli_fetch_assoc($q)) {
        ...
        if (in_array((int)$r['tipo_pregunta'], [3, 4], true)) {
            $qo = mysqli_query($con, "SELECT id_opcion, texto_opcion FROM encu_opciones WHERE id_pregunta = $idp AND baja = 0 ORDER BY nro_orden ASC, id_opcion ASC");
            while ($o = mysqli_fetch_assoc($qo)) $ops[] = ['id' => (int)$o['id_opcion'], 'texto' => eu_utf8($o['texto_opcion'])];
        }
        $items[] = [
            'id_pregunta'    => $idp,
            'nro_orden'      => (int)$r['nro_orden'],
            'texto'          => eu_utf8($r['texto_pregunta']),
            ...
            'area_nombre'    => eu_utf8($r['area_nombre']),
            ...
            'ref_texto'      => eu_utf8($r['ref_texto']),
            'opciones'       => $ops,
        ];
    }
    $out['encuesta'] = ['id_encuesta' => (int)$enc['id_encuesta'], 'nombre' => eu_utf8($enc['nombre']), 'activa' => (int)$enc['activa']];
    $out['areas']    = $areas;
    $out['items']    = $items;
