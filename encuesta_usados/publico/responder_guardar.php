<?php
/*
 * Guarda las respuestas de la encuesta pública de usados, calcula el promedio
 * ponderado y marca el token como completado. Responde JSON { ok } / { ok:false, error }.
 *
 * Ponderación (igual que 0km):
...
$rt = mysqli_query($con, "SELECT id_asignacion, id_encuesta FROM encu_tokens
...
$preguntas = [];
$rp = mysqli_query($con, "SELECT * FROM encu_preguntas WHERE id_encuesta = $id_encuesta AND baja = 0 ORDER BY nro_orden ASC");
while ($p = mysqli_fetch_assoc($rp)) {
    $p['opciones'] = [];
    if (in_array((int)$p['tipo_pregunta'], [3, 4], true)) {
        $ro = mysqli_query($con, "SELECT id_opcion FROM encu_opciones WHERE id_pregunta = {$p['id_pregunta']} AND baja = 0");
...
if (!mysqli_query($con, "INSERT INTO encu_respuestas (id_token, id_asignacion, id_encuesta)
...
foreach ($preguntas as $id_p => $preg) {
    ...
    mysqli_query($con, "INSERT INTO encu_respuestas_detalle (id_respuesta, id_pregunta, respuesta_valor, respuesta_texto, mostrada)
    ...
    if (!$omitida && $tipo == 3) {
        foreach ((array)($dato['opciones'] ?? []) as $idop) {
            ...
            if (isset($preg['opciones'][$idop]))
                mysqli_query($con, "INSERT INTO encu_respuestas_opciones (id_detalle, id_opcion, valor_elegido) VALUES ($id_detalle, $idop, 1)");
        }
    }
    if (!$omitida && $tipo == 4) {
        foreach ((array)($dato['opciones'] ?? []) as $item) {
            ...
            if (isset($preg['opciones'][$idop]))
                mysqli_query($con, "INSERT INTO encu_respuestas_opciones (id_detalle, id_opcion, valor_elegido) VALUES ($id_detalle, $idop, $val)");
        }
    }
...
mysqli_query($con, "UPDATE encu_respuestas SET resultado_promedio = $ps WHERE id_respuesta = $id_respuesta");
mysqli_query($con, "UPDATE encu_tokens SET completada = 1, fecha_respuesta = NOW() WHERE id_token = $id_token");
