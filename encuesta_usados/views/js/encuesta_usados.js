/*
 * Componente Alpine del módulo Encuesta de Satisfacción · Usados (tab Entregas).
 * Se carga al final del <body> (comun/layout.php). Carga data.php (JSON paginado)
 * y genera el link/QR de cada unidad vía token.php.
 */
function encuestaUsados(puedeConfigurar) {
  const columnas = [
    { key: 'fec_entrega', label: 'Entrega',  sortable: true,  width: '110px' },
    { key: 'cliente',     label: 'Cliente',  sortable: true,  width: '230px' },
    { key: 'vehiculo',    label: 'Vehículo', sortable: true,  width: '260px' },
    { key: 'dominio',     label: 'Dominio',  sortable: true,  width: '110px' },
    { key: 'asesor',      label: 'Asesor',   sortable: true,  width: '150px' },
    { key: 'id_sucursal', label: 'Sucursal', sortable: false, width: '120px' },
    { key: 'estado',      label: 'Encuesta', sortable: true,  width: '150px', align: 'center' },
  ...
  return {
    ...
    modal: { open: false, loading: false, error: '', cliente: '', vehiculo: '', asesor: '', entrega: '', link: '', token: '', copiado: false },
    ...
    async abrirToken(r) {
      this.modal = {
        ...
        cliente: r.cliente || '', vehiculo: r.vehiculo || '',
        asesor: r.asesor || '', entrega: this.fecha(r.fec_entrega),
    ...
    sucNombreId(id) { return (this.sucursales.find(s => s.id == id) || {}).nombre || ('Suc. ' + id); },
    estadoBadge(e) {
      return {
