...
<div class="zona-tabla ancho-95 s-100" style="overflow-y: scroll; padding-bottom: 0;">
	...
	<div class="zona-tabla ancho-95 s-100" style="height: 600px; overflow-y: auto; overflow-x: hidden;">
		<table class="ancho-95" style="margin-top: -73px;">
			<colgroup>
				<col width="12.6%">
				<col width="4.1%">
				<col width="4.1%">
			...
			<tbody>
				...
				<?php
					...
					while ($emple=mysqli_fetch_array($empleados)) { $cant_emp++;?>
						<tr>
							<td><span  style="cursor: pointer;" title="<?php echo  $emple['sector'].' - Sup. '.$emple['superior'].' - '.$emple['sucursal'] ?> "><?php echo '<b>'.$emple['nombre']."</b><br><span style='font-size: 11px;'>(".$emple['sector'].' - '.$emple['superior'].' - '.$emple['sucursal'].') </span>'; ?></span></td>
							...
							<?php
								...
								while ($item=mysqli_fetch_array($items)) { 
									...
									while ($resul=mysqli_fetch_array($resultados)) { if ($resul['auto']!=0) { $cant_item++; } ?>
										
										<td class="centrar-texto" style="">
											<div style="position: relative;"
											>
												<div>
													<?php if ($resul['auto']!= 0) { echo $resul['auto'];}else{ echo '-';}
													 if ($resul['auto']!= 0) {$auto=$auto+$resul['auto'];} ?>
												</div>
										...
										<td class="centrar-texto" style="background: #F1F1F1; color: black;">
											...
											>
												<div>
													<?php if ($resul['auto']!=0) { echo $resul['sup'];}else{ echo '-';}
													 if ($resul['auto']!=0) { $sup=$sup+$resul['sup']; } ?>
												</div>
