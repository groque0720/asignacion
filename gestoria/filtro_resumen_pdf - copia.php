...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(38,5,'Sucursal / Registro',1,0,'C');
		$columna=1;
	...
	if ($cant_tramites_p>0) {
		...
		while ($localidad = mysqli_fetch_array($localidades)) {
			...
			if ($cant_tramites_l>0) {
				...
				$pdf->Cell(38,5,'      '.utf8_decode(strtoupper($loc['localidad'])),1,0,'L');
				$cant_por_loc=0;
			...
			$pdf->Ln(2);
			$pdf->SetFont('Arial','B',7);
			$pdf->Cell(38,5,'Total '.utf8_decode(strtoupper($prov['provincia'])),1,0,'C');
			for ($i=0; $i <= $cant_grupo; $i++) { 
				$pdf->SetFont('Arial','B',7);
...
$pdf->Ln();
$pdf->SetFont('Arial','B',7);
$pdf->Cell(38,5,'TOTAL GRAL ',1,0,'C');
for ($i=0; $i <= $cant_grupo; $i++) { 
	$pdf->SetFont('Arial','B',7);
