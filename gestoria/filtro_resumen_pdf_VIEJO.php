...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(45,5,'Sucursal / Registro',1,0,'C');
		while ($grupo=mysqli_fetch_array($grupos)) {
			$this->Cell(16,5,$grupo['grupo'],1,0,'C');
		}
	...
	if ($cant_tramites_p>0) {
		...
		while ($localidad = mysqli_fetch_array($localidades)) {
			...
			if ($cant_tramites_l>0) {
				...
				$pdf->Cell(45,5,'      '.utf8_decode(strtoupper($loc['localidad'])),1,0,'L');
				$cant_por_loc=0;
				...
				$pdf->Cell(16,5,$cant_por_loc,1,0,'C');//total por localidad
				$pdf->Ln();
			...
			$pdf->Ln(2);
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(45,5,'Total '.utf8_decode(strtoupper($prov['provincia'])),1,0,'C');
			for ($i=0; $i <= $cant_grupo; $i++) { 
...
$pdf->Ln();
$pdf->SetFont('Arial','B',8);
$pdf->Cell(45,5,'TOTAL GRAL ',1,0,'C');
for ($i=0; $i <= $cant_grupo; $i++) { 
