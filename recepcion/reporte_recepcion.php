		...
		$pdf->Cell(40,5,'   '.$grupo_a[$grupo['idgrupo']]['grupo'],1,0,'L');
			...
			while ($modo = mysqli_fetch_array($modos)) {
				$SQL = "SELECT Count(recepcion.id_grupo) AS cantidad FROM recepcion
						 WHERE recepcion.id_sucursal =".$id." AND recepcion.fecha = '".$fecha."' AND recepcion.id_grupo = ".$grupo['idgrupo']." AND recepcion.id_acercamiento = ".$modo['id_modo_acercamiento'];
...
$grupos=mysqli_query($con, $SQL);
	...
	while ($grupo=mysqli_fetch_array($grupos)) {
		...
		$pdf->Cell(40,5,'   '.$grupo_a[$grupo['idgrupo']]['grupo'],1,0,'L');
			...
			while ($modo = mysqli_fetch_array($modos)) {
				$SQL = "SELECT Count(recepcion.id_grupo) AS cantidad FROM recepcion
						 WHERE recepcion.id_sucursal =".$id." AND MONTH(recepcion.fecha) = '".$mes_filtro."' AND recepcion.fecha <= '".$fecha."' AND YEAR(recepcion.fecha) = '".$anio."' AND recepcion.id_grupo = ".$grupo['idgrupo']." AND recepcion.id_acercamiento = ".$modo['id_modo_acercamiento'];
				$registros = mysqli_query($con, $SQL);
				$registro = mysqli_fetch_array($registros);
				if ($registro['cantidad']>0) {
					$pdf->Cell($ancho_modos,5,$registro['cantidad'],1,0,'C');
...
while ($sucursal = mysqli_fetch_array($sucursales)) {
	...
	while ($modo = mysqli_fetch_array($modos)) {
		...
		WHERE grupos.grupo <> '' AND grupos.activo = 1  AND recepcion.fecha = '".$fecha."' AND recepcion.id_acercamiento = ".$modo['id_modo_acercamiento']." AND
...
while ($sucursal = mysqli_fetch_array($sucursales)) {
	...
	while ($modo = mysqli_fetch_array($modos)) {
		$SQL="SELECT Count(recepcion.id_sucursal) AS cantidad FROM recepcion INNER JOIN grupos ON recepcion.id_grupo = grupos.idgrupo
		WHERE grupos.grupo <> '' AND grupos.activo = 1  AND MONTH(recepcion.fecha) = '".$mes_filtro."' AND recepcion.fecha <= '".$fecha."' AND YEAR(recepcion.fecha) = '".$anio."' AND recepcion.id_acercamiento = ".$modo['id_modo_acercamiento']." AND
		recepcion.id_sucursal=".$sucursal['idsucursal'];
		$registros = mysqli_query($con, $SQL);
		$registro = mysqli_fetch_array($registros);
		$pdf->Cell($ancho_modos,5,$registro['cantidad'],1,0,'C');
...
while ($sucursal = mysqli_fetch_array($sucursales)) {
	...
	while ($modo = mysqli_fetch_array($modos)) {
		$SQL="SELECT Count(recepcion.id_sucursal) AS cantidad FROM recepcion INNER JOIN grupos ON recepcion.id_grupo = grupos.idgrupo
		WHERE grupos.grupo <> '' AND grupos.activo = 1  AND recepcion.fecha <= '".$fecha."' AND YEAR(recepcion.fecha) = '".$anio."' AND recepcion.id_acercamiento = ".$modo['id_modo_acercamiento']." AND
		recepcion.id_sucursal=".$sucursal['idsucursal'];
		$registros = mysqli_query($con, $SQL);
		$registro = mysqli_fetch_array($registros);
		$pdf->Cell($ancho_modos,5,$registro['cantidad'],1,0,'C');
...
$pdf->Output('Reporte_Recepción','I');
$pdf->close();
