<?php
/*
 * Exportación Excel del filtro actual (situación + modelo + estado).
 * Wrapper auto-contenido: bootstrap moderno + consulta compartida + cuerpo PHPExcel
 * portado del módulo viejo (planes_avanzados/exportar_lista.php), sin cambios de formato.
 */
error_reporting(E_ERROR | E_PARSE);
@ini_set('display_errors', '0');

require __DIR__ . '/config/config_app.php';   // sesión + auth (redirect) + $con + $userId + $puedeEditar
require __DIR__ . '/funciones/consulta.php';   // tpa_*

if (!$puedeEditar) { header('Location: index.php'); exit(); }

require_once __DIR__ . '/Classes/PHPExcel.php';

list($situacionId, $modelo_activo_id, $estado) = tpa_filtros();
$modelosActivos = tpa_modelos_activos($con);
$idsModelos = array_column($modelosActivos, 'id');
if ($modelo_activo_id <= 0 || !in_array($modelo_activo_id, $idsModelos, true)) {
    $modelo_activo_id = $idsModelos[0] ?? 0;
}
$estadoId = $estado > 0 ? $estado : null;

// Nombre del modelo activo (para el nombre de archivo).
$modelo_activo_nombre = 'modelo';
foreach ($modelosActivos as $m) { if ($m['id'] === $modelo_activo_id) { $modelo_activo_nombre = $m['modelo']; break; } }

$planes_avanzados = tpa_query_planes($con, $situacionId, $modelo_activo_id, $estado);
