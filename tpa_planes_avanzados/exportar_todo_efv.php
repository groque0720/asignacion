<?php
/*
 * Exportación Excel de TODOS los modelos de la situación (exportar_todo_efv).
 * Wrapper auto-contenido: bootstrap moderno + consulta compartida + cuerpo PHPExcel
 * portado del módulo viejo (planes_avanzados/exportar_todo_efv.php), sin cambios de formato.
 */
...
$situacionId = isset($_GET['situacionId']) ? (int)$_GET['situacionId'] : 1;
$formato     = $_GET['formato'] ?? 'lista';

$planes_todos_modelos = tpa_query_planes($con, $situacionId, 0, 0);
if (!$planes_todos_modelos) { die('Error en la consulta: ' . mysqli_error($con)); }
