	<?php
	 ...
	 ?>
		...
		<script src="../js/ion.sound.js"></script>
	  	<script src="../js/noti_barra.js"></script>
...
<div class="noti_iconos" id="noti_iconos">
	...
	<div class="noti" id="noti_res">
		<div class="cantidad" id="cant_res">
			...
			<?php
				$SQL="SELECT count(*) as cantidad FROM notificaciones WHERE tiponot = 1 AND idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
	...
	<div class="noti" id="noti_act">
		<div class="cantidad" id="cant_act">
			<?php
				$SQL="SELECT count(tiponot) as cantidad FROM notificaciones WHERE tiponot = 2 and idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
				$res=mysqli_query($con, $SQL);
				if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
				if ($cant_res['cantidad']>0) {echo $cant_res['cantidad'];}
			 ?>
	...
	<div class="noti" id="noti_anu">
		<div class="cantidad" id="cant_anu">
			<?php
				$SQL="SELECT count(tiponot) as cantidad FROM notificaciones  WHERE tiponot = 3 AND idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
				$res=mysqli_query($con, $SQL);
				if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
				if ($cant_res['cantidad']>0) {echo $cant_res['cantidad'];}
	...
	<div class="noti" id="noti_fact">
		<div class="cantidad" id="cant_fact">
			<?php
				$SQL="SELECT count(tiponot) as cantidad FROM notificaciones WHERE tiponot = 4 AND idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
				$res=mysqli_query($con, $SQL);
				if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
				if ($cant_res['cantidad']>0) {echo $cant_res['cantidad'];}
	...
	<div class="noti" id="noti_cred">
		<div class="cantidad" id="cant_cred">
			<?php
				$SQL="SELECT count(tiponot) as cantidad FROM notificaciones WHERE tiponot = 5 AND idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
				$res=mysqli_query($con, $SQL);
				if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
				if ($cant_res['cantidad']>0) {echo $cant_res['cantidad'];}
			 ?>
	...
	<div class="noti" id="noti_canc">
		<div class="cantidad" id="cant_canc">
			<?php
				$SQL="SELECT count(tiponot) as cantidad FROM notificaciones WHERE tiponot = 6 AND idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
				$res=mysqli_query($con, $SQL);
				if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
				if ($cant_res['cantidad']>0) {echo $cant_res['cantidad'];}
			 ?>
...
<?php
	$SQL="SELECT count(tiponot) as cantidad FROM notificaciones WHERE idusuario =".$_SESSION["id"]." AND visto=0 and borrar=0";
	$res=mysqli_query($con, $SQL);
	if (empty($res)) {$cant_res['cantidad']=0;}else{ $cant_res=mysqli_fetch_array($res);}
