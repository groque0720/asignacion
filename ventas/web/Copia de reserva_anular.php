...
Inner Join grupos ON grupos.idgrupo = reservas.idgrupo
Inner Join modelos ON modelos.idmodelo = reservas.idmodelo
...
if ($res["enviada"]>0) {
	...
	$SQL .=" VALUES (3,'".date("Y-m-d")."','$hora','".$not["idusuario"]."','".$res["compra"]."','".$_GET["idres"]."','".$res["interno"].$_res["internou"]."','".$res['grupo']." ".$res['modelo']."".$res["detalleu"]."','".$res["cliente"]."','".$res["usuario"]."',0,'".$_GET["obser"]."')";
	mysqli_query($con, $SQL);
	};
