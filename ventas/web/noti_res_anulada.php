...
<div class="barra_btns">
	...
	<?php
		...
		conectar();
		$SQL="SELECT * FROM notificaciones WHERE tiponot = 3 AND idusuario =".$usu." AND borrar = 0 ORDER BY idnotificaciones DESC";
	...
	<table rules="all" border="1" id="tabla_res">
		...
		<?php
			$SQL="SELECT * FROM notificaciones WHERE tiponot = 3 AND idusuario =".$usu." AND borrar = 0 ORDER BY idnotificaciones DESC LIMIT ".$inicio.", $parametro_reg";
			$res=mysqli_query($con, $SQL);
		?>
