...
<div class="barra_btns">
	...
	<?php
		...
		conectar();
		$SQL="SELECT * FROM notificaciones WHERE tiponot = 3 AND idusuario =".$_GET["id"]." AND borrar = 0 ORDER BY idnotificaciones DESC";
	...
	<table rules="all" border="1" id="tabla_res">
		...
		<?php
			$SQL="SELECT * FROM notificaciones WHERE tiponot = 3 AND idusuario =".$_GET["id"]." AND borrar = 0 ORDER BY idnotificaciones DESC LIMIT ".$_GET["inicio"].", $parametro_reg";
			$res=mysqli_query($con, $SQL);
		?>
		...
		<thead>
			<tr>
				<td width="3%">N.Res.</td>
				<td width="5%">Fec.Anu.</td>
				<td width="4%">Hora</td>
				<td width="4%">Int.</td>
				<td width="13%">Modelo</td>
				<td width="15%">Cliente</td>
				<td width="6%">Asesor</td>
				<td width="15%">Motivo Anulaci&oacute;n </td>
				<td width="7%">Panel</td>
		...
		<tbody>
			...
			<tr class="<?php if ($noti_res["visto"]==0) { echo "negrita";} ?>">
				...
				<td >
					<a href="reserva.php?IDrecord=<?php echo $noti_res['idreserva']; ?>" data-id="<?php echo $nrofila; ?>" class="ir_reserva" id="ir"></a>
