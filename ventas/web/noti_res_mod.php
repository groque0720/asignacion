...
<div class="barra_btns">
	...
	<?php
		...
		conectar();
		$SQL="SELECT * FROM notificaciones WHERE tiponot = 2 AND idusuario =".$_GET["id"]." AND borrar = 0 ORDER BY idnotificaciones DESC";
	...
	<table rules="all" border="1" id="tabla_res">
		...
		<?php
			$SQL="SELECT * FROM notificaciones WHERE tiponot = 2 AND idusuario =".$_GET["id"]." AND borrar = 0 ORDER BY idnotificaciones DESC LIMIT ".$_GET["inicio"].", $parametro_reg";
			$res=mysqli_query($con, $SQL);
		?>
