...
<!--     // <script language="javascript" src="../js/jquery-1.10.2.js"></script> -->
	...
	$(document).ready(function(){
		...
		if ($("#enviado").val() >= 1) {
	   	...
	   	if ($("#idperfil").val()==14 || $("#idusuario").val()==11 || $("#idusuario").val()==56) {
			$("#menu_fact").show();
			};
...
//--------------------------------------------------------------------------------------------------
		...
		if ($("#modelo").val()!='' && $("#modelo").val()!= null && ($("#idperfil").val()==3 || $("#idusuario").val()==11 || $("#idusuario").val()==56)) {
   	 		$("#ingreso_codigo").show(0);
   		}else{

	   		 if ($("#detalleu").val()!='' && $("#detalleu").val()!= null && ($("#idperfil").val()==3 || $("#idusuario").val()==11)) {
	   	 		$("#ingreso_codigo").show(0);
	   		}else{
		...
		$('#enviar_res').click(function(event) {
			if (confirm("Confirma el envio de la Reserva??")) {
				if ($("#enviado").val() > 1) {
					...
					if (obs!="" && obs != null && obs.length>7) {
						$("#enviado").val(2)
						$("#obs_cambio").val(obs);

					}else{
						alert("Debe Ingresar un comentario válido para poder reenviar la Reserva");
						event.preventDefault();
					};
				...
				<form id="form_solicitud" name="form_solicitud" action="reserva_edit.php" method="POST" autocomplete="off">
					...
					<fieldset>
						...
						<div id="ingreso_codigo"  style="margin-bottom: 30px; display: flex" >
							...
							<div style="width:13%; float:left; text-align:left; display: flex;">
								<input type="text" id="monto" name="monto" placeholder="Monto" width="100%" style="text-align: right;">
							</div>
