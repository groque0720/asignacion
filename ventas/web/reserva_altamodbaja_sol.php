...
$SQL="SELECT * FROM lineas_detalle WHERE movimiento=1 AND idreserva='".$_POST['nrores']."'";
$der=mysqli_query($con, $SQL);


$SQL="SELECT * FROM lineas_detalle WHERE movimiento=2 AND idreserva='".$_POST['nrores']."'";
...
<div id="derecha">
	<table rules="all" border="1">
		...
		while ( $fila=mysqli_fetch_array($der)) { ?>
				<tr>
					...
					<td style="text-align:center;">
						...
						<?php if (($fila['idcodigo']<3) AND ($fila['moneda'] == 'U$S')) { echo 'U$S'; $band=1; } ?>
		...
		<tr>
			...
			<td style="text-align: right; font-weight: bold;"><?php if ($band == 0) {
					...
					<tr>
						...
						while ( $fila=mysqli_fetch_array($izq)) { ?>
							...
							<tr>
								...
								<td style="text-align: right; font-weight: bold;">Total a Pagar</td>
								<td style="text-align: right; font-weight: bold;"><?php if ($band == 0) {
									echo number_format($acumulado, 2, ',','.'); } ?></td>
								<td>.</td>
							</tr>
